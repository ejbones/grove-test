...
function gAddressPage(request, response){

	nlapiLogExecution('Audit', 'Grove Page Run', 'nb');
	...
	if(request.getParameter('custpage_addmth') == 'zipmth'){
		nlapiLogExecution('DEBUG', 'Zip method triggered', 'nb');
		stage = 'zip';
	}
	if(request.getParameter('custpage_addfree')){
		stage = 'addfreeresults';
		params += 'street=';
		params += encodeURI(request.getParameter('custpage_addfree'));
		res = nlapiRequestURL((surl + params), null, null, 'GET');
		nlapiLogExecution('DEBUG', 'Getting URL', (surl + params));
		rbody = res.getBody();
		nlapiLogExecution('DEBUG', 'Got free form address body', rbody);
		try{
			rbody = JSON.parse(res.getBody());
		}
		catch(e){
			nlapiLogExecution('ERROR', 'Error Parsing Free Address Response', e);
			//nlapiSendEmail(from, to, subject, body, cc, bcc, records, files, notifySenderOnBounce, internalOnly, replyTo);
			//send an email problem notification here if it's important
		}
		if(typeof rbody == 'object'){//finally we can find the shortest distance since we now have lat/long
			latres = rbody[0].metadata.latitude;
			longres = rbody[0].metadata.longitude;
			nlapiLogExecution('DEBUG', 'lat and long received', latres + ' | ' + longres);

			csvf = nlapiLoadFile(164846).getValue();
			nlapiLogExecution('DEBUG', 'After CSV get value', csvf);
			csvf = Papa.parse(csvf, {header:false,dynamicTyping:true});
			nlapiLogExecution('DEBUG', 'After CSV Parse', csvf.data[1][6]);
			for(var i = 1; i < csvf.data.length; i++){
				csvf.data[i].distance = PythagorasEquirectangular(latres, longres, csvf.data[i][6], csvf.data[i][7]);
				rA.push
				//nlapiLogExecution('DEBUG', 'FOUND DIF!', csvf.data[i].distance);
			}
			nlapiLogExecution('DEBUG', 'First distance before sort', csvf.data[1].distance);
			csvf.data.sort(function(a, b){return a.distance - b.distance});
			nlapiLogExecution('DEBUG', 'First distance after sort', csvf.data[1].distance);
			dresults.distance = csvf.data[1].distance;
			dresults.name = csvf.data[1][0];
			dresults.location = csvf.data[1][1];
			dresults.address = csvf.data[1][2];
			dresults.city = csvf.data[1][3];
			dresults.state = csvf.data[1][4];
			dresults.zip = csvf.data[1][5];
			
			//164846 csv file id - you could make this a record and import it via saved search, but i ran out of time to show that type of solution.
		}
	}
	if(request.getParameter('custpage_zip')){
		...
		rbody = res.getBody();
		nlapiLogExecution('DEBUG', 'Got zip body', rbody);
		try{
			rbody = JSON.parse(res.getBody());
		}
		catch(e){
			nlapiLogExecution('ERROR', 'Error Parsing Zip Response', e);
			//nlapiSendEmail(from, to, subject, body, cc, bcc, records, files, notifySenderOnBounce, internalOnly, replyTo);
			//send an email problem notification here if it's important
		}
		if(typeof rbody == 'object'){//finally we can find the shortest distance since we now have lat/long
			latres = rbody[0].zipcodes[0].latitude;
			longres = rbody[0].zipcodes[0].longitude;
			nlapiLogExecution('DEBUG', 'lat and long received', latres + ' | ' + longres);

			csvf = nlapiLoadFile(164846).getValue();
			//nlapiLogExecution('DEBUG', 'After ZIP CSV get value', csvf);
			csvf = Papa.parse(csvf, {header:false,dynamicTyping:true});
			nlapiLogExecution('DEBUG', 'After ZIP CSV Parse', csvf.data[1][6]);
			for(var i = 1; i < csvf.data.length; i++){
				csvf.data[i].distance = PythagorasEquirectangular(latres, longres, csvf.data[i][6], csvf.data[i][7]);
				rA.push
				//nlapiLogExecution('DEBUG', 'FOUND DIF!', csvf.data[i].distance);
	...
	var form = createGForm(stage, dresults, mikm, txtjsn);
	response.writePage(form);

}

function createGForm(stage, dresults, mikm, txtjsn){

	nlapiLogExecution('DEBUG', 'Creating Form, stage:', stage + ' ' + mikm + ' ' + txtjsn);

    var form = nlapiCreateForm('Grove Test',false);
    //form.setScript('customscript_some_script');
    ...
    if(stage == 'first load'){
    	...
    	form.addField('custpage_addfree', 'text', 'Full Address', 'type full address here properly!');
    	form.addField('custpage_mikm', 'radio', 'Use Miles', 'usemiles');
    	form.addField('custpage_mikm', 'radio', 'Use Kilometers', 'usekm');
    	form.getField('custpage_mikm', 'usemiles').setDefaultValue('usemiles');
    	form.addField('custpage_txtjsn', 'radio', 'Use Text', 'usetxt');
    	form.addField('custpage_txtjsn', 'radio', 'Use JSON', 'usejsn');
    	form.getField('custpage_txtjsn', 'usetxt').setDefaultValue('usetxt');
    	form.addSubmitButton('Next');
    }
    else if(stage == 'addfreeresults' || stage == 'zipresults'){
    	form.addField('custpage_resulttitle', 'label', 'Got response!', null, null);
    	if(mikm != 'usemiles'){
    		dresults.distance = dresults.distance * 1.609;
    	}
    	if(txtjsn == 'usetxt'){
    		nlapiLogExecution('DEBUG', 'txtjsn', txtjsn);
    		form.addField('custpage_results', 'label', 'Store name: ' + dresults.name + 'Distance: ' + dresults.distance + ' Location: ' + dresults.location + ' Address: ' + dresults.address + ' ' + dresults.city + ' ' + dresults.state + ' ' + dresults.zip, null, null);
    	}
    	else{
    		form.addField('custpage_results', 'label', 'JSON Results: ' + JSON.stringify(dresults), null, null);
    	}
    }
    else if(stage == 'zip'){
    	form.addField('custpage_zip', 'text', 'Zip Code', 'type zip code here!');
    	form.addField('custpage_mikm', 'radio', 'Use Miles', 'usemiles');
    	form.addField('custpage_mikm', 'radio', 'Use Kilometers', 'usekm');
    	form.getField('custpage_mikm', 'usemiles').setDefaultValue('usemiles');
    	form.addField('custpage_txtjsn', 'radio', 'Use Text', 'usetxt');
    ...
    form.addButton('custpage_cancel','Cancel','window.location=\'/app/center/card.nl?sc=-29\'');//returns user to home page
